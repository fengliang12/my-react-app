...
type VirtialListProps = {
     ...
     */
    onRender: (item: any, index: number, pageIndex: number) => JSX.Element
}


const VirtialList: React.FC<VirtialListProps> = (props) => {
    const { list = [], updateStatus, updateTopdata, updateCustom, rule = [{ screenCount: 99999 }], onRender } = props
    ...
    const computedWindowHeight = useMemoizedFn(() => {
        ...
        return systemInfoRef.current.windowHeight * currentRuleRef.current.screenCount
    })
    /** 格式化初始数据 */
    const formatList = useMemoizedFn((data) => {
        ...
        if (arr.length > 0) {
            ...
            pageHeightArrRef.current.push(arr.reduce((pre, cur) => pre + cur.height, 0))
        }
        updateTwoHeight()
        ...
        setTwoList((draft) => {
            _list?.forEach((item, index) => {
                if (currentIndexRef.current.includes(index)) {
                    draft[index] = {
                        ...item,
                        height: pageHeightArrRef.current?.length === 1 ? 'auto' : item.height,
                        isVirtial: false
                    }
                } else {
                    draft[index] = {
    ...
    const handleScroll = useMemoizedFn(({ scrollTop }) => {
        ...
        const currentIndex = twoHeightRef.current.findIndex(x => scrollTop > x[0] && scrollTop <= x[1])
        const arr: any = []
        if (toBottom) {
                ...
                setTwoList(draft => {
                    data.forEach(item => {
                        if (item.type === 'add') {
                            ...
                            if (!draft[item.index].list.filter(x => x.topData?.fixed)?.length) {
            ...
            updateStatus?.forEach(item => {
                draft.forEach((x, xIndex) => {
                    ...
                    if (myIndex !== -1) {
                        ...
                        const newInit = cloneDeep(initListRef.current)
