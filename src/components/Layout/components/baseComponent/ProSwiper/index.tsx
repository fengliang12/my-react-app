...
const ProSwiper: React.FC<ProSwiperProps> = ({ comInfo, comIndex }) => {
    ...
    return (<Swiper
        ...
        {...omit(comInfo.swiperView, ['current', 'indicatorDots', 'previousMargin', 'nextMargin'])}
        ...
        onChange={e => hanlderSwiperChange(e, { id: comInfo.id, relation: comInfo.relation }, swiperCurrent)}
    >
        {comInfo?.children?.map((x, index) => {
            const slot: any = slots?.find(y => y.index === index)
            return <>
                {iSlot && iSlot?.index === -1 && <SwiperItem>{iSlot.element}</SwiperItem>}
                <SwiperItem key={x.id}>
                    {
                        slot && slot.rerender && <>{slot.getElement(templateCustomData, swiperCurrent)}</>
                    }
                    {
                        slot && !slot.rerender && <><View style={{ position: 'absolute', left: slot.left, top: slot.top, bottom: slot.bottom, right: slot.right, zIndex: 1000 }}>{slot.getElement(templateCustomData, updateCurrent)}</View></>
                    }
                    {
                        !slot?.rerender && <>
                            {
                ...
                { iSlot && iSlot.index === index && <SwiperItem>{iSlot.element}</SwiperItem>}
            </>
        })}
    </Swiper>)
}
