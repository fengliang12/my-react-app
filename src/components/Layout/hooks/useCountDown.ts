...
import { useMount, useUpdateEffect, useMemoizedFn, useLatest } from 'ahooks';
import { useContext, useRef, useState } from 'react';
import { durationFormatter, formatDateTime, isBeforeValidTime } from '../helper';
import usePage from './usePage'
import { LiveTemplateContext, CountdownTemplateContext } from "../index"
...
export default function useCountDown(props) {
    ...
    const init = useMemoizedFn(() => {
        if (customData?.countDown) {
            ...
            const isShow = (countDown?.type === 'day' && countDown?.showDay !== 'none') || (countDown?.type === 'hour' && countDown?.showHour !== 'none') || (countDown?.type === 'minute' && countDown?.showMinute !== 'none') || (countDown?.type === 'second' && countDown?.showSecond !== 'none')
            if (isShow) {
                const fn = () => {
                    ...
                    if (result) {
                        ...
                        if (type === 'image') {
                            const src = getImgSrcByStr(result[countDown?.type].charAt(countDown?.numType))
        ...
        if (customData?.liveCountDown) {
            const fn = () => {
                ...
                if (result) {
                    Taro.nextTick(() => setValue(customData?.liveCountDown.prefix + result))
    ...
    const getCountDownValue = useMemoizedFn(() => {
        let { endTime, addZero, showDay, showHour, showMinute, type } = customData?.countDown
        if (countdownEndTime) {
            endTime = dayjs(countdownEndTime).format('YYYY-MM-DDTHH:mm:ssZ')
        }
        const isValid = dayjs().isBefore(endTime)
        if (isValid) {
            const duration: any = durationFormatter((new Date(formatDateTime(endTime, 6, '/')).getTime() - Date.now()) / 1000)
            let result: any = {
            ...
            if (duration.dd) {
                result.day = (duration.dd < 10 && (addZero || props?.type === 'image')) ? `0${duration.dd}` : `${duration.dd}`
            }
            if (duration.hh) {
                result.hour = (duration.hh < 10 && (addZero || props?.type === 'image')) ? `0${duration.hh}` : `${duration.hh}`
            }
            if (duration.mm) {
                result.minute = (duration.mm < 10 && (addZero || props?.type === 'image')) ? `0${duration.mm}` : `${duration.mm}`
            }
            if (duration.ss) {
                result.second = (duration.ss < 10 && (addZero || props?.type === 'image')) ? `0${duration.ss}` : `${duration.ss}`
            }
            ...
            if (showHour === 'none' && (duration.hh || duration.dd) && type === 'minute') {
                result.minute = `${((duration.dd || 0) * 24 + duration.hh || 0) * 60 + duration.mm || 0}`
                if (result.minute.length > 2 && props?.type === 'image') {
                    result.minute = '99'
                }
            }
            if (showMinute === 'none' && (duration.hh || duration.dd || duration.mm) && type === 'second') {
                result.second = `${(((duration.dd || 0) * 24 + duration.hh || 0) * 60 + duration.mm || 0) * 60 + duration.ss || 0}`
    ...
    const getLiveCountDownValue = useMemoizedFn(() => {
        ...
        if (isValid) {
            const duration: any = durationFormatter((new Date(formatDateTime(validTime?.[0], 6, '/')).getTime() - Date.now()) / 1000)
    ...
    const isNeedClearInterval = useMemoizedFn((countDown, result) => {
        ...
        if (type === 'text') {
            ...
            if (['00', '0'].includes(result[countDown?.type] as string)) {
                ...
                if (countDown?.type === 'minute' && ['00', '0'].includes(result.day) && ['00', '0'].includes(result.hour)) {
                    res = true
                }
            }
        }
        if (type === 'image') {
            ...
            res = (isNum0 && countDown?.type === 'day' && countDown?.numType === 0)
                || (isNum0 && countDown?.type === 'day' && result.day.charAt(0) === '0' && countDown?.numType === 1)
                || (isNum0 && countDown?.type === 'hour' && countDown?.numType === 0 && result.day.charAt(0) === '0' && result.day.charAt(1) === '0')
                || (isNum0 && countDown?.type === 'hour' && countDown?.numType === 1 && result.hour.charAt(0) === '0' && result.day.charAt(0) === '0' && result.day.charAt(1) === '0')
                || (isNum0 && countDown?.type === 'minute' && countDown?.numType === 0 && result.day.charAt(0) === '0' && result.day.charAt(1) === '0' && result.hour.charAt(0) === '0' && result.hour.charAt(1) === '0')
                || (isNum0 && countDown?.type === 'minute' && countDown?.numType === 1 && result.minute.charAt(0) === '0' && result.day.charAt(0) === '0' && result.day.charAt(1) === '0' && result.hour.charAt(0) === '0' && result.hour.charAt(1) === '0')
                || (isNum0 && countDown?.type === 'second' && countDown?.numType === 0 && result.minute.charAt(0) === '0' && result.minute.charAt(1) === '0' && result.day.charAt(0) === '0' && result.day.charAt(1) === '0' && result.hour.charAt(0) === '0' && result.hour.charAt(1) === '0')
        }
        return res
    })
    const isImgZero = useMemoizedFn((result: any) => {
        return customData?.countDown?.numType === 0 && result[customData?.countDown?.type].charAt(0) === '0' && !customData?.countDown?.addZero
    })
