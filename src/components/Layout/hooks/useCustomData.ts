...
export default function useCustomData(comInfo) {
  ...
  const animationInit = useMemoizedFn(() => {
    if (customData?.animation) {
      ...
      const fn = () => {
        Taro.nextTick(() => {
          const isAnimate = startsWith(
            customAnimation.style.animationName,
            'animate__'
          );
          if (isAnimate) {
          ...
          setCustomStyle(draft => {
            ...
            draft.animationIterationCount =
              customAnimation.style.animationIterationCount;
            draft.animationTimingFunction =
              customAnimation.style.animationTimingFunction;
          });
          const [nextTime, duration, delay] = [
            parseInt(customAnimation.nextTime ?? '0'),
            parseInt(customAnimation.style.animationDuration ?? '0'),
            parseInt(customAnimation.style.animationDelay ?? '0')
          ];
          if (
            nextTime &&
            customAnimation.style.animationDuration &&
            customAnimation.style.animationIterationCount !== 'infinite'
          ) {
            setTimeout(() => {
              if (isAnimate) {
                setCustomClassName('');
