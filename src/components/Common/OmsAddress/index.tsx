...
const OmsAddress: React.FC<propsType> = (props) => {
  ...
  const [rangeIndex, setRangeIndex] = useState<number[]>([0, 0, 0]);
  ...
  const getArea = () => {
    ...
    return (promiseArea = (async () => {
      ...
      const districtList = data.filter((item: any) => item.type === "area");
      return Object.freeze([provinceList, cityList, districtList]);
    })());
  };
