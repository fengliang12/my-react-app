(function () {
  ...
  var sourceToken = {
    ...
    rgbapercent:
      /rgba\(\s*(percent)[,\s]+(percent)[,\s]+(percent)(?:[,\s]*\/[,\s]*|[,\s]+)(unit)\)/,
  ...
  function replaceReg(regStr, name) {
    ...
    var result = regStr.replace(
      new RegExp(replaceTokens.join("|"), "g"),
      function (match) {
  ...
  function getReg(name) {
    return new RegExp(
      "^" + replaceReg(sourceToken[name].source, name) + "$",
      "i"
    );
  }
  ...
  function easycolor(color, options) {
    ...
    if (typeof type === "undefined" && typeof color === "object") {
      if (
        isMatch(color.r, "number") &&
        isMatch(color.g, "number") &&
        isMatch(color.b, "number")
      ) {
        type = "rgb";
      } else if (
        isMatch(color.r, "percent") &&
        isMatch(color.g, "percent") &&
        isMatch(color.b, "percent")
      ) {
        type = "rgb";
        percentage = true;
      } else if (
        isMatch(color.h, "hue") &&
        isMatch(color.s, "unit") &&
        isMatch(color.l, "unit")
      ) {
        type = "hsl";
      } else if (
        isMatch(color.h, "hue") &&
        isMatch(color.s, "unit") &&
        isMatch(color.v, "unit")
      ) {
        type = "hsv";
      }
    }
    ...
    if (type === "transparent") {
      ...
      val = hslToRgb(
        parseHue(color.h),
        parsePercent(color.s),
        parsePercent(color.l)
      );
    } else if (type === "hsv") {
      val = hsvToRgb(
        parseHue(color.h),
        parsePercent(color.s),
        parsePercent(color.v)
      );
    }
    ...
    this._percentage =
      typeof opt.percentage === "undefined"
        ? percentage
        : Boolean(opt.percentage);
  }
  ...
  easycolor.interpolation = function (start, end, count) {
    if (!(start instanceof easycolor) || !(end instanceof easycolor)) {
      throw new TypeError();
    }
    if (!count) return [];
    var colors = [start.clone()];
    if (count === 1) return colors;
    var step = divisionVector(
      subtractVector(end.getValue(), start.getValue()),
      count - 1
    );
    for (var i = 1; i <= count - 2; i++) {
      colors.push(
        easycolor(addVector(colors[colors.length - 1].getValue(), step))
      );
    }
  ...
  easycolor.mixAlpha = function (a, b) {
    ...
    var rgb = divisionVector(
      addVector(
        multiplyVector(color1, a1),
        multiplyVector(color2, a2 * (1 - a1))
      ),
      alpha
    );
    var val = {
  ...
  simpleExtend(easycolor.prototype, {
    ...
    toRgbPercentString: function () {
      ...
      return val.a === 1
        ? "rgb(" +
            rgbToPercent(val.r, decimalPoint) +
            "%," +
            rgbToPercent(val.g, decimalPoint) +
            "%," +
            rgbToPercent(val.b, decimalPoint) +
            "%)"
        : "rgba(" +
            rgbToPercent(val.r, decimalPoint) +
            "%," +
    ...
    toHslString: function () {
      ...
      return val.a === 1
        ? "hsl(" +
            toFixed(hsl.h, decimalPoint) +
            "," +
            toFixed(hsl.s, decimalPoint) +
            "%," +
            toFixed(hsl.l, decimalPoint) +
            "%)"
        : "hsla(" +
            toFixed(hsl.h, decimalPoint) +
            "," +
    ...
    toHsvString: function () {
      ...
      return val.a === 1
        ? "hsv(" +
            toFixed(hsv.h, decimalPoint) +
            "," +
            toFixed(hsv.s, decimalPoint) +
            "%," +
            toFixed(hsv.v, decimalPoint) +
            "%)"
        : "hsva(" +
            toFixed(hsv.h, decimalPoint) +
            "," +
